...
src/                          — Extension backend (Node.js, VS Code API)
  ...
  windowStatePersistence.ts   — Cross-window agent mirroring via ~/.pixel-agents/windows/*.json (one file per VS Code window)
...
**Cross-window agent mirroring**: Each VS Code window writes its own agent snapshot to `~/.pixel-agents/windows/<uuid>.json` (uuid generated per activation, cleaned up on deactivate). `windowStatePersistence.ts` handles I/O: `writeOwnState()` atomic tmp+rename, `watchWindowsDir()` fs.watch + 2s polling, `readRemoteWindows()` applies dedup (skip own file; skip files sharing own repoPath; per-repoPath last-write-wins). Files older than 5 min are treated as crashed and pruned. Snapshot includes `{id, palette, hueShift, seatId, isActive, currentTool, bubbleType}` per non-subagent character. Webview polls `OfficeState.characters` every 500ms and posts `updateWindowState` to the extension only when the JSON snapshot differs. To keep idle windows (or windows whose agents haven't changed state) from being pruned, the extension caches the last received snapshot and re-writes it every `WINDOW_STATE_HEARTBEAT_MS` (60s) so `updatedAt` stays fresh. After a PC sleep/wake, `watchWindowsDir()` detects large `Date.now()` jumps between polls (> `WINDOW_WAKE_TIME_JUMP_MS`) and passes `skipPrune=true` to `readRemoteWindows()` for that round, giving peers a grace period to refresh before their files are treated as crashed. Extension wraps with `{windowId, repoPath, repoName, updatedAt}` and writes to disk. Remote snapshots arrive in the webview as `remoteAgentsUpdated` messages; `OfficeState.setRemoteAgents()` diff-spawns/updates/despawns entries in `remoteCharacters` (keyed `${windowId}:${agentId}`, synthetic id starts at `-1_000_000` to avoid sub-agent collision). Remote characters run the standard FSM locally — only the high-level `seatId/isActive/currentTool/bubbleType` is synced. `remoteOccupiedSeats` (rebuilt each tick + on snapshot) excludes remote-taken seats from `findFreeSeat()` and `pickRestSeat()`. `getAllCharactersForRender()` merges local + remote for the renderer; hit-testing in `getCharacterAt()` also includes remotes so hover overlays work, but the `OfficeCanvas` click handler early-returns when the hit id isn't in local `characters` (remote avatars are observational only — no selection/close/focus/terminal action). `findAnyCharacter(id)` unifies lookup across local + remote for `ToolOverlay`. Remote `folderName` is set to the sender's repoName so the overlay shows which window they came from; for remote agents, activity text and dot color are derived from mirrored scalar fields (`isActive`, `currentTool`, `bubbleType`) since the per-agent tool-activity list isn't synced.
...
**Sound notifications**: Ascending two-note chime (E5 → E6) via Web Audio API plays when waiting bubble appears (`agentStatus: 'waiting'`). `notificationSound.ts` manages AudioContext lifecycle; `unlockAudio()` called on canvas mousedown to ensure context is resumed (webviews start suspended). Toggled via "Sound Notifications" checkbox in Settings modal. Enabled by default; persisted in extension `globalState` key `pixel-agents.soundEnabled`, sent to webview as `settingsLoaded` on init. **Window ownership filter**: sounds only fire for agents launched via this window's "+ Agent" button. External agents (`isExternal: true` — teammates, externally-adopted sessions, hooks-only providers) are tracked in `externalAgentIdsRef` on the webview from `existingAgents.externalAgents` + `agentCreated.isExternal`, and both `playDoneSound`/`playPermissionSound` skip for them. **Replay suppression**: `sendCurrentAgentStatuses` re-emits `agentStatus: 'waiting'` with `isReplay: true` on webview reinit; the webview skips the chime when `isReplay` is set or when the prior status was already `waiting` (via `lastAgentStatusRef`).
